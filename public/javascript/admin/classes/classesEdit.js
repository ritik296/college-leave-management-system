...
let tableRowCount = tableRowContainer.children.length;

// console.log(tableRowCount)

const classJson = {};

function tableRow(tableRowCount){
    return `<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                <input id="teacher-id-${tableRowCount}" class="w-full border-none focus:outline-none" type="text" id="teacher-id-" placeholder="Enter Teacher ID">
            </th>
            <td class="py-4 px-6">
                <input id="subject-name-${tableRowCount}" class="w-full border-none focus:outline-none" type="text" id="subject-name-" placeholder="Enter Subject Name">
            </td>
            <td class="py-4 px-6">
                <input id="subject-code-${tableRowCount}" class="w-full border-none focus:outline-none" type="text" id="subject-code-" placeholder="Enter Subject Code">
            </td>
            <td class="py-4 px-6">
                <button class="p-1 rounded-full bg-red-500 px-2 text-white font-bold" onclick="removeRow(${tableRowCount})">Remove</button>
            </td>`;
}
