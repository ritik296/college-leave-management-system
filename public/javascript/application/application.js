...
const blockHeading = document.getElementById('block-heading');
...
function applictionCard({leave, reason, subject, file, from, to, createdAt, status, studentDetail, _id}){
    let s = new Date(from);
    let e = new Date(to);
    // <!--onclick="toggleCard('${createdAt}')"-->
    let html = `<div id='${createdAt}card' class="card p-5 rounded-xl border mb-0 shadow-inner shadow-current ${status === "pending"? 'bg-yellow-100': status === 'approved'? 'bg-green-100': 'bg-red-100'}" onclick="selectCard('${studentDetail.name}', '${createdAt}')">
                    <div class="head-of-application flex justify-center items-center">
                        <div class="left sm:w-5/6 w-2/3">
                            <h1 class="text-xl text-[#263238] font-bold">${studentDetail.name}</h1>
                            <h1 class="text-sm text-[#263238]">${subject}</h1>
                        </div>
                        <div class="right sm:w-1/6 w-1/3">
                            <p class="text-sm font-bold text-[#263238] text-right">${createdAt.slice(0, 10)}&nbsp; ${createdAt.slice(11, 16)}</p>
                            <p class="text-sm font-bold text-[#263238] text-right">${status}</p>
                        </div>
                    </div>
                    <div id="${createdAt}" class="body-of-application hidden ">
                        <p class="text-md text-[#263238] mt-4">${reason}</p>
                        <p class="text-md text-[#263238] mt-4">I request you to kindly provide me ${leave} days leave from <b>${s.getDate()}/${s.getMonth() +1}/${s.getFullYear()}</b> to <b>${e.getDate()}/${e.getMonth() +1}/${e.getFullYear()}</b>. I shall be greatful.</p>
                        <p class="text-md text-[#263238] mt-3">Thanking You</p>
                        <p class="text-md text-[#263238] mt-1">Your faithfully</p>
                        <p class="text-md text-[#263238] mt-1">${studentDetail.name}</p>
                        <p class="text-md text-[#263238] mt-1">${studentDetail.contact}</p>
                        <p class="text-xs text-[#263238] mt-1">${studentDetail.branch} branch ${studentDetail.semester} sem ${studentDetail.section}th section</p>
                        ${file?
                            `<a class="flex" href="/${file}" target="_blank">
                                <p class="p-3 bg-white mt-2 border rounded-full shadow-md">
                                    Attached file 
                                </p>
                            </a>`: ""
                        }
                        ...
                        `<div class="flex justify-around">
                            <button class="w-2/5 h-14 bg-gradient-to-tr from-emerald-600 to-emerald-400 mt-6 rounded-xl text-white font-bold text-xl hover:shadow-md hover:shadow-emerald-400" onclick="sendApplicationResponce('${_id}', true)">APPROVE</button>
                            <button class="w-2/5 h-14 bg-gradient-to-tr from-red-600 to-red-400 mt-6 rounded-xl text-white font-bold text-xl hover:shadow-md hover:shadow-red-400" onclick="sendApplicationResponce('${_id}', false)">REJECT</button>
                        </div>` : ""
                        }
                    </div>
                </div>`;
...
async function getApplication() {
    let res = await fetch('/api/v1/application/get_recived_application', {
        method: 'GET',
        headers: {
    ...
    if(res.status == 200){
        ...
        // console.log(resData);
        applicationsData = resData.data;
        setCard();
        closeSpinner();
    }
    if(res.status == 404){
        noticationContainer.innerHTML = noApplicationFound();
        closeSpinner();
    }
}
getApplication();
...
function setCard(data){
    let html = '';
    for(let ele of applicationsData.application){
        // console.log(ele);
        html += applictionCard(ele);
    }
    // console.log("done");
    
    applicationContainer.innerHTML = html;
    totalApplication.innerHTML = `Total Applications: - ${applicationsData.totalApplication}`;
    totalPending.innerHTML = `Total Pending: - ${applicationsData.totalPending}`;
    totalApproved.innerHTML = `Total Approved: - ${applicationsData.totalApproved}`;
    totalReject.innerHTML = `Total Rejected: - ${applicationsData.totalReject}`;
}

function toggleCard(eleId){
    const cardBody = document.getElementById(eleId);
    const card = document.getElementById(`${eleId}card`);
    if(cardBody.classList.contains('hidden')){
        cardBody.classList.remove('hidden');
        card.classList.add('2xl:col-span-2');
    } else{
        cardBody.classList.add('hidden');
        card.classList.remove('2xl:col-span-2');
    }
}

async function sendApplicationResponce(id, state) {
    // console.log(id, " ", state);
    openSpinner();
    let res = await fetch('/api/v1/application/responce_application', {
        method: 'PUT',
        body: JSON.stringify({
            "applicationId": id,
            "status": state? 'approved': 'rejected'
        }),
        headers: {
            'Content-type': 'application/json; charset=UTF-8', 'auth-token': `${sessionStorage.getItem('auth-token')}`
        }
    ...
    if(res.status == 401) {
        ...
        let resData = await res.json();
        alert(`Application ${state ? "Approved": "Rejected"}`);
        closeSpinner();
        location.reload(); 
    }
    else if(res.status == 404){
        alert('No application found with this details.');
        closeSpinner();
    }
}


function noApplicationFound(){
    return `<div class="text-5xl font-dark font-bold">404</div>
...
function selectCard(name, id){
    ...
    if(!toggle){
        for(let ele of applicationsData.application){
            if(ele.studentDetail.name !== name){
        ...
        studentNameElement.innerText = name;
        blockHeading.innerText = `${name}`
        blockNavigation.innerHTML = `<span class="text-[#607d8b] text-sm antialiased font-sans font-normal leading-normal mx-2 pointer-events-none select-none">/</span>
                                    <span onclick="backApplication()">  
                                        <p class="block antialiased font-sans text-sm leading-normal text-[#263238] font-normal">Applications</p>
                                    </span>
                                    <span class="text-[#607d8b] text-sm antialiased font-sans font-normal leading-normal mx-2 pointer-events-none select-none">/</span>
                                    <span>  
                                        <p class="block antialiased font-sans text-sm leading-normal text-[#263238] font-normal">${name}</p>
                                    </span>`
    }
    toggleCard(id);
}
...
function backApplication(){
    if(toggle){
        for(let ele of applicationsData.application){
            const cardBody = document.getElementById(ele.createdAt);
            const card = document.getElementById(`${ele.createdAt}card`);
        ...
        totalApproved.innerHTML = `Total Approved: - ${applicationsData.totalApproved}`;
        totalReject.innerHTML = `Total Rejected: - ${applicationsData.totalReject}`;
        studentNameElement.innerText = "Students";
        blockHeading.innerText = `Home`;
        blockNavigation.innerHTML = `<span class="text-[#607d8b] text-sm antialiased font-sans font-normal leading-normal mx-2 pointer-events-none select-none">/</span>
                                    <span onclick="backApplication()">  
                                        <p class="block antialiased font-sans text-sm leading-normal text-[#263238] font-normal">Applications</p>
                                    </span>`;
