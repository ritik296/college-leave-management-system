...
function applictionCard({leave, reason, subject, file, from, to, createdAt, status, studentDetail, _id}){
    let s = new Date(from);
    let e = new Date(to);
    let html = `<div id='${createdAt}card' class="card p-5 rounded-xl border mb-0 shadow-inner shadow-current ${status === "pending"? 'bg-yellow-100': status === 'approved'? 'bg-green-100': 'bg-red-100'}" onclick="toggleCard('${createdAt}')">
...
function setCard(data){
    let html = '';
    for(let ele of data.application){
        // console.log(ele);
    ...
    totalApplication.innerHTML = `Total Applications: - ${data.totalApplication}`;
    totalPending.innerHTML = `Total Pending: - ${data.totalPending}`;
    totalApproved.innerHTML = `Total Approved: - ${data.totalApproved}`;
    totalReject.innerHTML = `Total Rejected: - ${data.totalReject}`;
}
